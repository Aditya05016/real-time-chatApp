...
import { ChatContext } from '../../context/ChatContext';
 ...
 const SideBar = () => {
    ...
    const [input,setInput] = useState(false)
      ...
      useEffect(()=>{
        if(onlineUsers.length > 0){ //mew line added
        getUsers();
      }
      },[onlineUsers])
  return (
    ...
    text-white ${selectedUser ? "max-md:hidden" :'' }`}>
        ...
        <div className='flex flex-col'>
            {filterdUsers.map((user,index)=>(
                <div onClick={()=>{setSelectedUser(user)}} key={index} className= {`relative flex items-center gap-2 p-2 pl-4 rounded cursor pointer max-sm:text-sm${selectedUser?.id === user._id && "bg-[#282142]/50"}`}>
