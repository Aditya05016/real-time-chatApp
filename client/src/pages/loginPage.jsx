 ...
 const LoginPage = () => {
  ...
  return (
    ...
    sm:justify-evenly max-sm:flex-col backdrop-blur-2xl">
      {/*..........left} */}
      <img src={assets.logo_big} alt="" className="w-[min(90vw,300px)]" />  
      {/*......right */} 
      ...
      flex-col gap-6 rounded-lg shadow-lg">
        <h2 className="font-medium text-lg flex justify-between items-center">
          {currState}
          {isDataSubmitted && <img onClick={() => setDataSubmitted(false)} src={assets.arrow_icon} alt=""
          ...
          <div className="flex items-center gap-2 text-sm text-gray-500">
            <input type = "checkbox" />
            <p>Agree to the therm of use & privacy policy.</p>
          </div>
          <div className="flex flex-col gap-2">
            {currState === "Sign up" ? (
              <p className="text-sm text-gray-600">Alredy have an account? 
              <span onClick={()=>{setCurrState("Login"); setDataSubmitted(false)}} className="font-medium text-violet-500 cursor-pointer">Login here</span></p>
            ) : (
