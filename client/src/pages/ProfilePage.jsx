...
import assets from '../assets/assets'; // or wherever your assets file is
 ...
 const ProfilePage = () => {
  ...
  const[bio,setBio] = useState(authUser.bio)
   ...
   const handleSubmit = async(e) => {
    ...
    if(selectedImg){
      profilePicBase64 = await readFileAsDataURL(selectedImg);
    }

    await updateProfile({profilePic: profilePicBase64, fullName: name, bio});
     setSelectedImg(null);
    navigate('/');
  }
      ...
      rounded-lg'>
        ...
        className={`w-12 h-12 ${selectedImg && 'rounded-full'}`}/>
          ...
          <input onChange={(e)=>setName(e.target.value)} value ={name} 
           type = "text" placeholder = "Your name" className='p-2 border
